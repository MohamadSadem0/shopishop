...
public class NotificationServiceImpl {
    ...
    // Send a notification to all superadmins subscribed to the WebSocket topic
    public void sendNotificationToSuperAdmin(String message) {
        // Broadcast the notification to all super admins subscribed to the topic
        messagingTemplate.convertAndSend("/topic/superadmin-notifications", message);
        System.out.println("Notification sent to superadmin: " + message);
    }

    public void notifyMerchant(String email, String message) {
        messagingTemplate.convertAndSendToUser(email, "/topic/store-approval", message);
    }
}
