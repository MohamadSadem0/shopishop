  ...
  return (
    <div className={`fixed top-0 right-0 w-80 h-screen bg-white shadow-lg p-6 ${cartOpen ? 'block' : 'hidden'}`}>
      <div className="flex justify-between items-center mb-4">
        <h2 className="text-lg font-semibold text-color1">Your Cart</h2>
          ...
          <>
            {cartItems.map((item, index) => (
              <div key={index} className="flex items-center justify-between mb-4">
                ...
                <div>
                  <p className="text-color1">{item.name}</p>
            ...
            <div className="mt-4">
              <p className="font-semibold text-color1">Total: ${getTotalPrice()}</p>
            </div>
          </>
        )}
      </div>
